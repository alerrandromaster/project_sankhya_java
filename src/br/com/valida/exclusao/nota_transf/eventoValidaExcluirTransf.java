package br.com.valida.exclusao.nota_transf;
...
public class eventoValidaExcluirTransf implements EventoProgramavelJava {
	...
	public void afterInsert(PersistenceEvent arg0) throws Exception {
		...
		EntityFacade dwfEntityFacade = EntityFacadeFactory.getDWFFacade();
		 ...
		 if ( codtipoper.compareTo(BigDecimal.valueOf(9020)) == 0 ) {
			
			StringBuilder LibAreas = new StringBuilder();
			LibAreas.append(" select 	  \r\n"
					+ "CASE \r\n"
					+ "   WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (1,9,12,13,18,23) THEN 1055\r\n"
					+ "  WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (6,19,24) THEN 1056\r\n"
					+ "WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (7,10,14,15,20,25) THEN 1057\r\n"
					+ " WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (8,11,16,17,21,26) THEN 1058   end AS EVENTO \r\n"
					+ "from tgfite where nunota = " + numerodaNota +" and sequencia > 0\r\n"
					+ "AND sankhya.REC_ASEP(CODLOCALORIG, CODPROD) IS NOT NULL\r\n"
					+ "GROUP BY\r\n"
					+ "CASE WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (1,9,12,13,18,23) THEN 1055\r\n"
					+ " WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (6,19,24) THEN 1056\r\n"
					+ " WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (7,10,14,15,20,25) THEN 1057\r\n"
					+ " WHEN sankhya.REC_ASEP(CODLOCALORIG,CODPROD) IN (8,11,16,17,21,26) THEN 1058\r\n"
					+ " END");
	...
	private void proibirExclusao(PersistenceEvent arg0) throws Exception {
		...
		if (Confirmada.equals("A") && statusnota.equals("L") && TipMov.equals("T") ) {
